...
--11
select department from (select count(*) as c,department from employees group by department) where c>2;

--12
select distinct on(budget) name from departments order by budget desc limit 1 offset 1;

--13
select name, lastname from employees where department=(select distinct on(budget) code from departments limit 1);
...
--16
update departments set budget=budget*0.9 where code=(select distinct on(budget) code from departments limit 1);
